...
TIDE_MIN_CROSSING_M = 1.7   # minimum water height for breakwater crossing
...
class Simulation:
    def __init__(self):
        ...
        for vv in self.vessels:
            if vv.name == "Bedford":
                ...
                # Balham starts berthing Alkebulan at BIA, full load
                ...
                self.log_event(0, vv.name, "LOADING_START",
                               f"Loading {_wk_cap:,} bbl | {STORAGE_QUATERNARY_NAME}: "
        ...
        # ── Berth reservations for all vessels active at t=0 ─────────────────
        # Balham → Alkebulan: berthing at t=0, hose at 0.5h, discharge 0.5..14.5h
        _balham_end  = BERTHING_DELAY_HOURS + HOSE_CONNECTION_HOURS + DISCHARGE_HOURS
        self.mother_berth_free_at["Alkebulan"] = _balham_end
        # Bagshot → Bryanston: hose already connected at t=0, discharge 0..12h
        _bagshot_end = HOSE_CONNECTION_HOURS + DISCHARGE_HOURS   # fires at t=0 → done at 14h
        self.mother_berth_free_at[MOTHER_PRIMARY_NAME] = _bagshot_end
        # Watson → GreenEagle: hose already connected at t=0, discharge 0..12h
        _watson_end  = HOSE_CONNECTION_HOURS + DISCHARGE_HOURS
        self.mother_berth_free_at[MOTHER_TERTIARY_NAME] = _watson_end
        # Serial discharge gate: all three discharge windows overlap, so gate is the
        # latest end across all three mothers — next vessel may not berth until then.
        self.next_mother_berthing_start_at = max(_balham_end, _bagshot_end, _watson_end)
    ...
    def tide_ok_at(self, hour):
        ...
        return h is None or h >= TIDE_MIN_CROSSING_M
    ...
    def next_tidal_sail(self, current_hour):
        ...
        return t   # fallback: return last computed even if tide marginal
        ...
        while t <= total_hours:
            ...
            for v in self.vessels:
                ...
                if v.status == "IDLE_A":
                    ...
                    # Vessel receives a full cargo from storage — its API is the storage's current API
                    self.vessel_api[v.name] = self.storage_api.get(selected_storage, 0.0)
                    v.cargo_bbl = cap
                    ...
                    if not candidates:
                        ...
                        # ── Dynamic highest-stock assignment ──────────────────────────────
                        # Rule: always assign the arriving daughter to the mother with the
                        # highest current stock so that vessel reaches export trigger first.
                        # When multiple daughters arrive on the same calendar day, each
                        # successive arrival takes the next-highest mother (tracked via
                        # point_b_day_allocation_count so the ranking is consumed in order
                        # of arrival without re-sorting mid-day).
                        # Tie-break: alphabetical name (deterministic across runs).
                        ...
                        # Rank ALL eligible mothers by stock descending; tie-break by name.
                        ranked_mothers = sorted(
                            candidate_by_mother.keys(),
                            key=lambda name: (-self.mother_bbl[name], name),
                        )

                        # Pick the slot corresponding to today's arrival count.
                        # If more vessels arrive than there are mothers, wrap around
                        # (guards against edge-case where all mothers are partially full).
                        slot_idx = assigned_today % len(ranked_mothers)
                        selected_mother = ranked_mothers[slot_idx]
                        ...
                        self.log_event(
                            ...
                            f"Day {day_key + 1} arrival slot {assigned_today + 1}: assigned to "
                            f"{selected_mother} (highest-stock rule) | "
                            f"stock ranking: {', '.join(f'{n} {self.mother_bbl[n]:,.0f} bbl' for n in ranked_mothers)}",
                            voyage_num=v.current_voyage,
                        )
                    ...
                    if self.mother_bbl[selected_mother] + v.cargo_bbl > MOTHER_CAPACITY_BBL:
                        ...
                        self.mother_api[selected_mother] = self.blend_api(
                            ...
                            v.cargo_bbl, self.vessel_api.get(v.name, 0.0))
