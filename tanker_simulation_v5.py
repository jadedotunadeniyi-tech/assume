...
# Structure: {vessel_name: {day_key_0based (int): mother_name (str)
...
# Date-keyed overrides: when a "date" key is present, the override fires on
# the calendar date regardless of how many sim days have elapsed, providing
# a stable reference that does not shift with the simulation start date.
...
class Simulation:
    ...
    def _resolve_discharge_override(self, vessel_name: str, t: float) -> "str | None":
        """Return the forced mother name for vessel_name at sim time t, or None.

        Supports two override formats stored in DAUGHTER_DISCHARGE_OVERRIDES:
          • Legacy:  {vessel: {day_key_0based: "MotherName"}}
          • Date:    {vessel: {day_key_0based: {"mother": "MotherName",
                                                "date": "YYYY-MM-DD",
                                                "voyage_code": "XXX-NNN"}}}

        For date-keyed overrides, the match fires when the calendar date at
        sim time t equals the stored ISO date string, making the override
        immune to simulation start-date changes.
        """
        vessel_overrides = DAUGHTER_DISCHARGE_OVERRIDES.get(vessel_name, {})
        if not vessel_overrides:
            return None
        current_cal_date = self.hours_to_dt(t).date().isoformat()
        day_key = int(t // 24)
        # Scan all entries — prefer date match over day_key match when both exist
        for _dk, _entry in vessel_overrides.items():
            if isinstance(_entry, dict):
                _date = _entry.get("date")
                _mother = _entry.get("mother")
                if _date and _mother:
                    if _date == current_cal_date:
                        return _mother
            else:
                # Legacy plain-string format keyed by day_key
                if int(_dk) == day_key:
                    return _entry
        return None
                ...
                if v.status == "IDLE_A":
                    ...
                    if not candidates:
                        ...
                        if STARTUP_DAY_DISABLE_POINT_B_PRIORITY and day_key == 0:
                            ...
                            # or {vessel: {day: {"mother": ..., "date": "YYYY-MM-DD"}}}
                            # Forces this daughter to a specific mother on a given
                            # sim day or calendar date.  ZeeZee is unaffected (she
                            # is handled by _run_zeezee / ZEEZEE_SCHEDULE entirely
                            # separately).
                            _ddo_mother = self._resolve_discharge_override(v.name, arrival)
                            ...
                            if _ddo_mother and _ddo_mother in _candidate_mothers:
                                ...
                                self.log_event(
                                    ...
                                    f"on {self.hours_to_dt(arrival).strftime('%Y-%m-%d')} (user rule)",
                                ...
                                self.log_event(
                                    ...
                                    f"Discharge point override targets {_ddo_mother} on "
                                    f"{self.hours_to_dt(arrival).strftime('%Y-%m-%d')} "
                    ...
                    if STARTUP_DAY_DISABLE_POINT_B_PRIORITY and day_key == 0:
                        ...
                        _ddo_mother = self._resolve_discharge_override(v.name, decision_t)
                        _candidate_mothers = {m for _, _, m in candidates}
                        if _ddo_mother and _ddo_mother in _candidate_mothers:
                            selected_mother = _ddo_mother
                            selected = next(
                            ...
                            if _ddo_mother != v.assigned_mother:
                                self.log_event(
                                    ...
                                    f"Discharge point override: forced to {_ddo_mother} "
                                    f"on {self.hours_to_dt(decision_t).strftime('%Y-%m-%d')} (user rule)",
                                    voyage_num=v.current_voyage, mother=_ddo_mother,
                                )
                        elif _ddo_mother and _ddo_mother not in _candidate_mothers:
                            ...
                            self.log_event(
                                ...
                                f"Discharge point override awaiting {_ddo_mother} on "
                                f"{self.hours_to_dt(decision_t).strftime('%Y-%m-%d')}; "
