...
def run_sim(sim_days, chapel, jasmines, westmore, duke, starturn, pgm,
    ...
    # Structure: {vessel_name: {day_key_0based: mother_str
    #                           OR {"mother": str, "date": "YYYY-MM-DD", "voyage_code": str}}}
    # Date-keyed entries match by calendar date in the sim (immune to start-date changes).
        ...
        try:
            ...
            if isinstance(_raw_ddo, dict):
                for _vn, _day_map in _raw_ddo.items():
                    if isinstance(_day_map, dict):
                        _parsed_map = {}
                        for _d, _entry in _day_map.items():
                            if isinstance(_entry, dict):
                                # New date-keyed format: preserve dict as-is
                                _parsed_map[int(_d)] = _entry
                            else:
                                # Legacy plain-string format
                                _parsed_map[int(_d)] = str(_entry)
                        mod.DAUGHTER_DISCHARGE_OVERRIDES[_vn] = _parsed_map
...
<div class="summary-card">
  ...
  <ul>
    ...
    # Maps voyage_code → {vessel, day_1based} for the "By Voyage Code" lookup.
    _vcode_map: dict = {}
    try:
        if not log_df.empty and "VoyageCode" in log_df.columns:
            ...
            for _, _lrow in _ls.iterrows():
                ...
                if _vc_key and _vc_vn and _vc_key not in _vcode_map:
                    _vcode_map[_vc_key] = {"vessel": _vc_vn, "day": _vc_day}
    except Exception:
        pass
    ...
    with st.expander(
        ...
        st.markdown(
            ...
            'Reference a voyage by its short <b>Voyage Code</b> (e.g. <code>BGT-007</code>) '
            'shown in the JMP table, or use the vessel/day selectors. '
        ...
        # ── Voyage Code quick-lookup ──────────────────────────────────────────
        if _vcode_map:
            st.markdown(
                '<div style="background:#f0f9ff;border:1px solid #38bdf8;border-radius:6px;'
                'padding:8px 12px;margin-bottom:10px">'
                '<span style="font-size:11px;font-weight:700;color:#0369a1">🔖 VOYAGE CODE LOOKUP</span>'
                '<span style="font-size:11px;color:#64748b;margin-left:6px">— type a voyage code to '
                'auto-fill vessel and day below</span></div>',
                unsafe_allow_html=True,
            )
            _vc_lookup_col, _vc_info_col = st.columns([2, 4])
            with _vc_lookup_col:
                _vc_input = st.text_input(
                    "Voyage Code",
                    value="",
                    key="ddo_voyage_code_lookup",
                    placeholder="e.g. BGT-007",
                    help="Enter the 7-character voyage code from the JMP table (e.g. SHK-001). "
                         "Vessel and day selectors below will be pre-filled automatically.",
                ).strip().upper()
            with _vc_info_col:
                if _vc_input and _vc_input in _vcode_map:
                    _vc_resolved = _vcode_map[_vc_input]
                    _vc_vn  = _vc_resolved["vessel"]
                    _vc_col = VESSEL_COLORS.get(_vc_vn, "#64748b")
                    st.markdown(
                        f'<div style="padding-top:26px">'
                        f'<span style="background:{_vc_col};color:#fff;border-radius:4px;'
                        f'padding:3px 9px;font-size:12px;font-weight:700">{_vc_vn}</span> '
                        f'<span style="font-size:12px;color:#374151"> · loaded Day '
                        f'<b>{_vc_resolved["day"]}</b> · discharges ~Day '
                        f'<b>{_vc_resolved["day"] + 1}</b></span></div>',
                        unsafe_allow_html=True,
                    )
                elif _vc_input:
                    st.markdown(
                        '<div style="padding-top:26px;font-size:12px;color:#ef4444">'
                        '⚠️ Code not found — run simulation first, then look up.</div>',
                        unsafe_allow_html=True,
                    )
                else:
                    st.markdown(
                        '<div style="padding-top:26px;font-size:12px;color:#94a3b8">'
                        f'{len(_vcode_map)} voyage codes available from last run.</div>',
                        unsafe_allow_html=True,
                    )
            # Pre-fill defaults from lookup if matched
            _ddo_default_vessel = _vcode_map.get(_vc_input, {}).get("vessel", _ddo_all_vessels[0]) if _vc_input else _ddo_all_vessels[0]
            _ddo_default_day    = min(max(1, _vcode_map.get(_vc_input, {}).get("day", 1) + 1), max(_ddo_sim_days, 1)) if _vc_input else 1
            _ddo_vessel_idx     = _ddo_all_vessels.index(_ddo_default_vessel) if _ddo_default_vessel in _ddo_all_vessels else 0
        else:
            _ddo_vessel_idx  = 0
            _ddo_default_day = 1
            st.caption("💡 Tip: run the simulation once to populate voyage codes for easy lookup.")

        st.markdown("**Add discharge rule** — by vessel/date or auto-filled from voyage code above:")
        _ddo_c1, _ddo_c2, _ddo_c3, _ddo_c4 = st.columns([2, 2, 2, 1])
        with _ddo_c1:
            _ddo_vessel = st.selectbox(
                "Daughter vessel",
                options=_ddo_all_vessels,
                index=_ddo_vessel_idx,
                key="ddo_vessel",
                help="Daughter vessel to override. Auto-filled when a valid voyage code is entered above.",
            )
        with _ddo_c2:
            # Calendar date: default to sim start date + (default_day) days
            _ddo_sim_start = sim_start_date if isinstance(sim_start_date, _dt.date) else _dt.date.today()
            _ddo_default_date = _ddo_sim_start + _dt.timedelta(days=max(0, _ddo_default_day - 1))
            _ddo_max_date = _ddo_sim_start + _dt.timedelta(days=max(_ddo_sim_days - 1, 0))
            _ddo_arrival_date = st.date_input(
                "Date vessel arrives at BIA",
                value=_ddo_default_date,
                min_value=_ddo_sim_start,
                max_value=_ddo_max_date,
                key="ddo_date",
                help="Calendar date the vessel arrives at BIA to discharge. "
                     "This date is matched exactly against the simulation calendar, "
                     "so it stays correct even if you change the simulation start date.",
            )
        with _ddo_c3:
            _ddo_mother = st.selectbox(
                ...
                key="ddo_mother",
                help="Mother vessel this daughter must discharge to on the selected date.",
            )
        with _ddo_c4:
            st.markdown("<br>", unsafe_allow_html=True)
            if st.button("➕ Add", key="ddo_add_btn", use_container_width=True):
                ...
                # Compute 0-based day key from arrival date relative to sim start
                _arrival_dt = _ddo_arrival_date if hasattr(_ddo_arrival_date, "isoformat") else _ddo_arrival_date
                _dk_from_date = (_arrival_dt - _ddo_sim_start).days
                # Store as dict with date + mother so sim can match by calendar date
                _upd_ddo[_ddo_vessel][int(_dk_from_date)] = {
                    "mother":      str(_ddo_mother),
                    "date":        _arrival_dt.isoformat(),
                    "voyage_code": str(st.session_state.get("ddo_voyage_code_lookup", "")).strip().upper(),
                }
        ...
        if _ddo_state:
            ...
            for _ddo_vn, _ddo_day_map in sorted(_ddo_state.items()):
                for _ddo_dk, _ddo_entry in sorted(_ddo_day_map.items(), key=lambda x: int(x[0])):
                    # Support both old plain-string format and new dict format
                    if isinstance(_ddo_entry, dict):
                        _ddo_mn   = _ddo_entry.get("mother", "?")
                        _ddo_date = _ddo_entry.get("date", "")
                        _ddo_vc   = _ddo_entry.get("voyage_code", "")
                        _ddo_label_date = _ddo_date if _ddo_date else f"Day {int(_ddo_dk)+1}"
                    else:
                        _ddo_mn   = str(_ddo_entry)
                        _ddo_date = ""
                        _ddo_vc   = ""
                        _ddo_label_date = f"Day {int(_ddo_dk)+1}"
                    _vc_col  = VESSEL_COLORS.get(_ddo_vn, "#064e3b")
                    # Voyage code badge (from stored entry or looked up from map)
                    if not _ddo_vc:
                        _ddo_vc = next(
                            (vc for vc, info in _vcode_map.items()
                             if info["vessel"] == _ddo_vn
                             and abs(info["day"] - (int(_ddo_dk))) <= 1),
                            "",
                        )
                    _rule_code_badge = (
                        f'<span style="background:#0f172a;color:#7dd3fc;border-radius:3px;'
                        f'padding:1px 7px;font-size:10px;font-weight:700;'
                        f'font-family:monospace;margin-left:5px">{_ddo_vc}</span>'
                        if _ddo_vc else ""
                    )
                    ...
                    with _dc1:
                        st.markdown(
                            ...
                            f'{_rule_code_badge} '
                            f'<span style="font-size:12px;color:#374151">'
                            f' <b>{_ddo_label_date}</b> → discharge to '
                            f'<b>{_ddo_mn}</b></span>',
                            unsafe_allow_html=True,
                        )
                    with _dc2:
