...
def run_sim(sim_days, chapel, jasmines, westmore, duke, starturn,
    ...
    if sim_start_date:
        _epoch = _dtmod2.date.fromisoformat(sim_start_date)
    else:
        _epoch = _dtmod2.date.today()
    ...
    if opt_params_json:
        _opt_pr = _json_rs.loads(opt_params_json)
    ...
    for _v in sim.vessels:
        ...
        sim.log_event(0, _v.name, "LOADING_START",
                      f"Loading {_vcap:,} bbl | {_stor}: {_stor_stock_now:,.0f} bbl "
...
def run_optimizer(base_params_json: str):
    ...
    """
    import json as _json, itertools as _it, math as _math

    base = _json.loads(base_params_json)
    _tide_hex   = base.pop("_tide_csv_bytes_hex", None)
    _tide_bytes = None
    if _tide_hex:
        import binascii as _ba2
        _tide_bytes = _ba2.unhexlify(_tide_hex)
    ...
    def _score(S, log_df, tl_df):
        ...
        if len(cyc_ev) >= 2:
            ...
            avg_cycle = _math.fsum(gaps)/len(gaps) if gaps else 48.0
        else:
            avg_cycle = 48.0
        ...
        for sn in ["Chapel","JasmineS","Westmore","Duke","Starturn"]:
            if sn in tl_df.columns:
                col = tl_df[sn].dropna()
                cap_col = f"{sn}_cap"
                cap = 290_000 if sn in ("JasmineS","Westmore") else 228_000
                su[sn] = {
    ...
    return _json.dumps(best_slim), tbl.to_json(orient="records")
...
def main():
    ...
    with st.sidebar:
        ...
        sim_start_date = st.date_input(
            ...
            value=_today,
            min_value=_dtmod.date(2020, 1, 1),
            max_value=_dtmod.date(2035, 12, 31),
            format="DD/MM/YYYY",
            help=(
...
""")
        ...
        tide_file = st.file_uploader(
            ...
            help=(
                ...
                "Vessels cross the breakwater only if tide ≥ 1.7 m within daylight.\n"
        ...
        if tide_file is not None:
            ...
            st.caption("🌊 Min crossing depth: 1.7 m · daylight only (06:00–18:00)")
        else:
    ...
    if not gs_vols:
        with st.expander("✏️ Enter 08:00 storage & mother volumes",
            ...
            for j,(sn,cv) in enumerate([
                ("Chapel",270_000),("JasmineS",290_000),("Westmore",290_000),
    ...
    import datetime as _dtmod_call
    _start_iso_str = sim_start_date.isoformat() if hasattr(sim_start_date, "isoformat") else _dtmod_call.date.today().isoformat()
    ...
    if run_opt and _sel_scen:
        _opt_params_for_run = _json_main.dumps({
    ...
    elif run_opt and 'best_pr' in dir():
        # After optimization runs, default to best scenario params
        _opt_params_for_run = _json_main.dumps(best_pr)
    else:
    ...
    try:
        _today_date = _dt2.date.fromisoformat(_start_iso_str)
    except Exception:
        _today_date = _dt2.date.today()
    ...
    def _pcargo(detail):
        m = _re2.search(r"([\d,]+) bbl", detail)
        return int(m.group(1).replace(",", "")) if m else 0

    def _pstorage_from_detail(detail):
        m = _re2.search(r"\| (\w+):", detail)
    ...
    for _vn in ALL_VESSELS:
        ...
        if _st in _LOAD_ST:
            ...
            if _storage == "?" and not _bev.empty:
                ...
                _m = _re2.search(r"(?:at|window at) (\w+)", _d)
            ...
            if not _rev_all.empty:
                ...
                _d = _rev_use["Detail"]
                _m2 = _re2.search(r"eligible storage: (\w+)", _d)
                if _m2:
                    _ret_stor = _m2.group(1)
                else:
                    # Fallback: derive storage from target_point in detail
                    _mp = _re2.search(r"Allocated to Point ([A-F])", _d)
            ...
            if not _mev.empty:
                ...
                _m = (_re2.search(r"assigned to (\w+)", _detail)
                      or _re2.search(r"Hose connected at (\w+)", _detail)
                      or _re2.search(r"\|\s*(\w+):", _detail))
...
Generated {_dt2.datetime.now().strftime('%Y-%m-%d %H:%M')} | Tanker Operations Simulation v5
</div>
</body></html>"""
    ...
    if _tide_bytes is None:
        ...
        try:
            import csv as _csv_t, re as _re_t
            _tide_min_m = 1.7
            _DSTART     = 6
            _DEND       = 18
            _sim_epoch  = _dt_tide.datetime(_today_date.year, _today_date.month, _today_date.day, 8, 0)  # t=0 = 08:00
            ...
            def _parse_tide_bytes(raw_bytes, epoch_dt):
                ...
                reader = _csv_t.DictReader(_io_t.StringIO(text), delimiter=delim)
                ...
                for row in rows:
                    try:
                        ...
                        if "/" in ds:
                            p = ds.split("/")
                            d = (_dt_tide.datetime(int(p[2]),int(p[1]),int(p[0]))
                                 if len(p[2])==4
                                 else _dt_tide.datetime(int(p[0]),int(p[1]),int(p[2])))
                        else:
                            d = _dt_tide.datetime.fromisoformat(ds.split("T")[0])
                        hh, mm = int(ts[:2]), int(ts[3:5])
                        dt  = d.replace(hour=hh, minute=mm)
                        ht  = float(_re_t.sub(r"[^0-9.\-]", "", hs))
            ...
            if not _tide_ok:
                ...
                for _day in range(_sim_days_t):
                    _date_d    = _today_date + _dt_tide.timedelta(days=_day)
                    ...
                    if not _slots:
                        _daily_rows.append({
                            ...
                            "High Tide": "—", "High Time": "—",
                            "Low Tide": "—", "Low Time": "—",
                            "Crossing Windows (daylight ≥1.7m)": "No tidal data for this day",
                            "Windows": 0,
                        })
                        continue
                    ...
                    _peak_dt   = (_sim_epoch + _dt_tide.timedelta(hours=_peak_h)).strftime("%H:%M")
                    _trough_dt = (_sim_epoch + _dt_tide.timedelta(hours=_trough_h)).strftime("%H:%M")

                    # Crossing windows: consecutive slots where height >= threshold AND daylight
                    _DSTART = 6
                    _DEND   = 18
                    _windows = []
                    _win_start = None
                    for _h in sorted(_slots):
                        ...
                        _valid = (_slots[_h] >= _tide_min_m) and (_DSTART <= _hod < _DEND)
                        if _valid and _win_start is None:
                            _win_start = _h
                        elif not _valid and _win_start is not None:
                            _win_end = _h
                            _ws = (_sim_epoch + _dt_tide.timedelta(hours=_win_start)).strftime("%H:%M")
                            _we = (_sim_epoch + _dt_tide.timedelta(hours=_win_end)).strftime("%H:%M")
                            _wh = round(_win_end - _win_start, 1)
                            _windows.append(f"{_ws}–{_we} ({_wh}h)")
                            _win_start = None
                    ...
                    _daily_rows.append({
                        ...
                        "Low Time":  _trough_dt,
                        "Crossing Windows (daylight ≥1.7m)": _win_str,
                ...
                st.markdown(
                    ...
                    f'🌊 <b>Breakwater crossing rule:</b> vessels may only depart when tide ≥ '
                ...
                if _sd_slots:
                    ...
                    _in_zone = False
                    _zone_x0 = None
                    for _hod, _hgt in zip(_sd_hods, _sd_heights):
                    ...
                    _cwin_txt = _sel_row["Crossing Windows (daylight ≥1.7m)"]
                    st.caption(
    ...
    if not run_opt:
        ...
        if _tide_bytes is not None:
            ...
            base_params_for_opt["_tide_csv_bytes_hex"] = _binascii.hexlify(_tide_bytes).decode()
        base_params_for_opt["_sim_start_date"] = _start_iso_str if _start_iso_str else ""
        opt_cache_key = _json.dumps(base_params_for_opt, sort_keys=True)

        with st.spinner("🔍 Running optimization sweep (daylight-constrained scenarios)…"):
            best_json, tbl_json = run_optimizer(opt_cache_key)

        best_r  = _json.loads(best_json)
        import io as _io_opt
        opt_tbl = pd.read_json(_io_opt.StringIO(tbl_json), orient="records")
        ...
        _tide_html_text  = (
            "🌊 Tidal Constraint Active — breakwater ≥ 1.7 m · 2h from SanBarth · 4h from BIA"
            if _tide_bytes else
        ...
        best_export = {
            ...
            "scores":         best_sc,
            "all_scenarios":  _json.loads(tbl_json),
        }
        st.download_button(
            "📥 Download Optimization Report (JSON)",
            data=_json.dumps(best_export, indent=2),
    ...
    # ==========================================================================
    import datetime as _dtmod_hdr
    _start_lbl = _dtmod_hdr.date.fromisoformat(_start_iso_str).strftime('%-d %b %Y') if _start_iso_str else 'Today'
    ...
    _capacities  = {"Chapel":270_000,"JasmineS":290_000,"Westmore":290_000,"Duke":90_000,"Starturn":70_000}
...
<div class="summary-card">
  ...
  <ul>
    ...
    SanBarth→BIA breakwater require ≥1.7 m tide during daylight. Departures must
    ...
    storage_items = [("Chapel","A",270_000),("JasmineS","A",290_000),
                     ("Westmore","C",290_000),("Duke","D",90_000),("Starturn","E",70_000)]
    ...
    st_info = [
        ...
        ("Westmore","Westmore_bbl","Westmore_Overflow_Accum_bbl",290_000,"C",sorted(mod.WESTMORE_PERMITTED_VESSELS)),
    ...
    import re as _re
    import datetime as _dtjmp
    import io as _io

    # ── Helper: derive plan start date ────────────────────────────────────────
    try:
        _jmp_start = _dtjmp.date.fromisoformat(_start_iso_str)
    except Exception:
        _jmp_start = _dtjmp.date.today()
    ...
    def _parse_cargo(detail):
        m = _re.search(r"([\d,]+) bbl", detail)
        return int(m.group(1).replace(",","")) if m else 0

    def _parse_storage(detail):
        m = _re.search(r"\| (\w+):", detail)
        return m.group(1) if m else ""
...
<style>
  ...
  .jmp-bia-entry{display:inline-block;border-radius:4px;padding:2px 6px;
    ...
    for _day in range(1, _jmp_days + 1):
        _date = _jmp_start + _dtjmp.timedelta(days=_day - 1)
        _de = _ev[_day]
        _stocks  = _de["stocks"]
...
<div style="font-size:9px;color:#94a3b8;margin-top:8px">Generated: {_dtjmp.datetime.now().strftime("%Y-%m-%d %H:%M")} | Tanker Operations v5</div>
</body></html>"""
    ...
    with d3:
        rows = [
        ["Simulation Start Date", _dtmod_hdr.date.fromisoformat(_start_iso_str).strftime('%d/%m/%Y')],
